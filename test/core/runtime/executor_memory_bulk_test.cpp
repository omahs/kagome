...
class BulkMemoryTest : public testing::Test {
 public:
  ...
  void SetUp() override {
    ...
    auto code_provider = std::make_shared<BasicCodeProvider>(
        ...
        + "/wasm/fill_memory_bulk_enabled.wasm");
  ...
  outcome::result<RuntimeContext> prepareCall(
    ...
    EXPECT_CALL(*module_instance,
                callExportFunction(_, std::string_view{"fill_memory"}, _))
...
TEST_F(BulkMemoryTest, LatestStateSwitchesCorrectly) {
...
//   EXPECT_OUTCOME_TRUE(
//       ctx1,
//       prepareCall(block_info1, "state_hash1"_hash256, CallType::Persistent));
//   EXPECT_TRUE(executor.call<void>(ctx1, "fill_memory") == outcome::success());
  auto ctx1 = prepareCall(block_info1, "state_hash1"_hash256, CallType::Persistent);
  auto module_instance = std::make_shared<ModuleInstanceMock>();
  module_instance->callExportFunction(ctx1, "fill_memory", Buffer{});
}
